...
import codecs
import socket
import requests #urllib.request doesn't support socks5 proxy. A DNS error may happen!

proxies={'http':'socks5h://127.0.0.1:1080','https':'socks5h://127.0.0.1:1080'} # IP address should be modified according to your own proxy setting.

# based on google-images-download project (https://google-images-download.readthedocs.io/en/latest/index.html)
# only argument"keywords","color type" and "limit" are used in our web page. The others may not be supported.
...
def user_input():
    ...
    parser.add_argument('-wr', '--time_range',help='time range for the age of the image. should be in the format {"time_min":"YYYY-MM-DD","time_max":"YYYY-MM-DD"}',type=str, required=False)
...
class googleimagesdownload:
    ...
    def _extract_data_pack(self, page):
        ...
        #bytes(object_raw, "utf-8").decode("unicode_escape")
        return object_raw
    ...
    def download_page(self, url):
        ...
        try:
            ...
            respData = str(Response.text)
        except:
            print("Could not open URL. Please check your internet connection/ssl setting/proxy setting")
            sys.exit()
        try:
            iopf = self._image_objects_from_pack(self._extract_data_pack(respData)), self.get_all_tabs(respData)
            #print(iopf)
            return iopf
        ...
        params = {'color': [arguments['color'], {'red': 'ic:specific,isc:red', 'orange': 'ic:specific,isc:orange',
                  ...
                  'size': [arguments['size'],
                           {'large': 'isz:l', 'medium': 'isz:m', 'icon': 'isz:i', '>400*300': 'isz:lt,islt:qsvga',
                            ...
                            '>1024*768': 'visz:lt,islt:xga', '>2MP': 'isz:lt,main_directory', '>4MP': 'isz:lt,islt:4mp',
    ...
    def download_image(self, image_url, image_format, main_directory, dir_name, count, print_urls, socket_timeout,
        ...
        try:
            Response = requests.get(image_url, headers={
                "User-Agent": "Mozilla/5.0 (X11; Linux i686) AppleWebKit/537.17 (KHTML, like Gecko) Chrome/24.0.1312.27 Safari/537.17"},proxies=proxies)
            try:
                ...
                try:
                    ...
                    # output_file.close()
                    with open(path, "wb") as f:  # 保存的文件名 保存的方式（wb 二进制  w 字符串）
                        f.write(data)
                    if save_source:
    ...
    def download_executor(self, arguments):
        ...
        limit = int(arguments['limit'])# Setting limit on number of images to be downloaded
        if(limit>30):
            print("limit should be less than 31!")
            return
        ...
        while i < len(search_keyword):  # 3.for every main keyword
            ...
            # create directory & build url
            search_term = search_keyword[i]
            dir_name = search_term  # sub-directory
            ...
            #download page
            images, tabs = self.download_page(url)

            # download image
            print("Starting Download...")
            items, errorCount, abs_path = self._get_all_items(images, main_directory, dir_name, limit,arguments)
